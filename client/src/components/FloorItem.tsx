...
export default function FloorItem({ floor, key, active }: { floor: Floor, key: Key | null | undefined, active?: boolean }) {
    let containerStyles: CSSProperties = { transition: "0.5s all ease-out", transform: "rotateY(8deg)"  }
    ...
    return <div key={key} style={{ display: "flex",  ...containerStyles }}>
        <div style={{ display: "flex", justifyContent: "center", fontSize: 35, fontWeight: 700, padding: 20, minWidth: 80, ...leftContainerStyles }}>{floor.num}</div>
        
        <div className="grid grid-cols-2 p-5" style={{ flexGrow: 1, gap: 20, ...rightContainerStyles }}>
